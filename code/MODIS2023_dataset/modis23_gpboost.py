...
def vecchia_run(num_neighbors):
    ...
    #saving results
    filename = "modis23_vecchia_" + str(num_neighbors)
...
def fitc_run(num_ind_points):
    ...
    crps_test = np.mean([crps_gaussian(mu, sigma, x) for mu, sigma, x in zip(pred_mean_test, np.sqrt(pred_var_test), y_test)])
    #saving results
    filename = "modis23_fitc_" + str(num_ind_points)
    with open(filename + '.txt', 'w') as file:
...
def tapering_run(cov_fct_taper_range):
    ...
    crps_test = np.mean([crps_gaussian(mu, sigma, x) for mu, sigma, x in zip(pred_mean_test, np.sqrt(pred_var_test), y_test)])
    #saving results
    filename = "modis23_tapering_" + str(cov_fct_taper_range)
    with open(filename + '.txt', 'w') as file:
...
def fullscale_run(cov_fct_taper_range, num_ind_points):
    ...
    #saving results
    filename = "modis23_fullscale_" + str(cov_fct_taper_range) + "_" + str(num_ind_points)
    with open(filename + '.txt', 'w') as file:
